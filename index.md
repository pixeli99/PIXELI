  ...
  <li{% if exc.size > 10 %} class="with-excerpt"{% endif %}>
    <div class="entry-head">
      ...
      <span class="entry-tail">
        ...
        {% if item.collection == "papers" and item.authors %}{% assign _aenc = item.authors | url_encode %}<span class="entry-authors"{% unless _aenc contains '%E' %} lang="en"{% endunless %} title="{{ item.authors }}">{{ item.authors }}</span>{% endif %}
        ...
        {% if item.collection == "notes" and item.tags.size > 0 %}{% for t in item.tags limit: 2 %}{% assign _tenc = t | url_encode %}<a href="{{ '/tags/' | relative_url }}#tag-{{ t | replace: ' ', '-' }}" class="entry-tag"{% unless _tenc contains '%E' %} lang="en"{% endunless %}>{{ t }}</a>{% endfor %}{% endif %}

...
<div class="papers-filter" role="group" aria-label="按标签过滤">
  <button type="button" class="pf-btn pf-active" data-tag="" aria-pressed="true" aria-label="全部，{{ papers_sorted.size }} 篇">全部<sup aria-hidden="true">{{ papers_sorted.size }}</sup></button>
  {% for row in _freq_rows %}{% assign _parts = row | split: "|" %}{% assign _n = _parts[0] | plus: 0 %}<button type="button" class="pf-btn" data-tag="{{ _parts[1] }}" aria-pressed="false"><span lang="en">{{ _parts[1] }}</span><sup aria-hidden="true">{{ _n }}</sup><span class="sr-only">，{{ _n }} 篇</span></button>{% endfor %}
</div>

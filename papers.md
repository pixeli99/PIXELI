...
{% for p in papers_sorted %}
  ...
  {% unless years contains yr %}{% assign years = years | push: yr %}{% endunless %}
{% endfor %}

<nav class="year-nav" aria-label="按年份跳转">
{% for y in years %}<a href="#y-{{ y }}">{{ y }}</a>{% unless forloop.last %} · {% endunless %}{% endfor %}
</nav>
...
{% for p in papers_sorted %}
  ...
  {% if year != prev_year %}
    ...
    <h2 class="section-label" id="y-{{ year }}">{{ year }}</h2>
    <ul class="entry-list">

...
{% for group in papers_by_year %}<a href="#y-{{ group.name }}" aria-label="{{ group.name }} 年，{{ group.items.size }} 篇">{{ group.name }}<sup aria-hidden="true">{{ group.items.size }}</sup></a>{% unless forloop.last %} · {% endunless %}{% endfor %}
</nav>

...
<div class="papers-filter" role="group" aria-label="按标签过滤">
  ...
  {% for row in _freq_rows %}{% assign _parts = row | split: "|" %}<button class="pf-btn" data-tag="{{ _parts[1] }}" aria-pressed="false">{{ _parts[1] }}</button>{% endfor %}
</div>
...
(function () {
  ...
  if (initialTag) setActive(initialTag);
})();
</script>
